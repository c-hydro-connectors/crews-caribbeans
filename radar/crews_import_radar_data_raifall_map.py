...
def main(): #main(path, in_dir, out_dir, plot, out_flag):
    ...
    for time_now in date_range:
        ...
        if presenceZ>0:
            ...
            differenza = float(rbdict['product']['data']['radarpicture']['@max']) - float(rbdict['product']['data']['radarpicture']['@min'])
            data_rain = ((data_rain_bin * differenza) / 255) + float(rbdict['product']['data']['radarpicture']['@min'])

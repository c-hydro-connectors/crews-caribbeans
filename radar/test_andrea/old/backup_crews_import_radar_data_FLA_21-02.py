...
def main(): #main(path, in_dir, out_dir, plot, out_flag):
    ...
    for time_now in date_range:
        ...
        try:
            ...
            logging.error("ERROR! File " + filename + " not found")
        ...
        if presenceZ>0:
            ...
            da_rain.to_netcdf(os.path.join(folder_out_now, "temp_" + file_out_now))
            ...
            if data_settings["data"]["dynamic"]["output"]["format"] == "tif":
                file_out_now_tiff = data_settings["data"]["dynamic"]["output"]["filename_tif"].format(**template_filled)
                os.system('gdal_translate -if netcdf -of GTiff -co COMPRESS=DEFLATE ' + os.path.join(folder_out_now, "temp_" + file_out_now) + ' ' + os.path.join(folder_out_now, file_out_now_tiff))
            elif data_settings["data"]["dynamic"]["output"]["format"] == "netcdf":
                os.system('cp ' + os.path.join(folder_out_now, "temp_" + file_out_now) + ' ' + os.path.join(folder_out_now, file_out_now))
            else:
                logging.error("ERROR! Only tif and netcdf output types supported!")
                raise NotImplementedError
            
            os.system('rm ' + os.path.join(folder_out_now, "temp_" + file_out_now))
